"""MCP client wrappers for connecting to MCP servers and custom tools."""
...
class MCPClient:
    ...
    def extend_tools(self, custom_tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def set_custom_tool_executor(
        self,
        executor: Callable[[str, dict[str, Any]], Union[str, Any]],
    ):
...
class MultiMCPClient:
    ...
    def add_custom_tools(
        ...
        """Add custom tools with an executor function."""
    ...
    async def _get_server_tools(
        ...
        try:
            ...
            logger.warning("Failed to get tools from %s: %s", server_name, e)
            return []
        ...
        for server_name, server_params in self.servers.items():
            if name.startswith(f"{server_name}_"):
                ...
                try:
                    ...
                    logger.warning("Error calling %s on %s: %s", name, server_name, e)
